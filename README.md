...
Запускаю Claude Code в Docker-контейнере с полной изоляцией. Клод делает что хочет внутри контейнера, но наружу не видит ничего, кроме папки `claude_in_box/`.
...
```bash
./claude-docker                  # новый сеанс
./claude-docker --resume         # продолжить последний
./claude-docker --continue       # продолжить с места остановки
./claude-docker -p "что делает этот код?"  # один вопрос
```
...
| Команда | Помнит диалог? | Для чего |
|---------|---------------|----------|
| `./claude-docker` | Нет | Новая задача с нуля |
| `./claude-docker --resume` | Да, весь | Продолжить вчерашнее |
| `./claude-docker --continue` | Да, с места остановки | Перезапустил контейнер и хочешь дальше |
| `./claude-docker -p "..."` | Нет | Быстрый вопрос |

Авто-память (файлы проектов, CLAUDE.md) работает во всех режимах.
