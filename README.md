...
#### Windows: `no such file or directory` при запуске

Git на Windows по умолчанию конвертирует переносы строк LF → CRLF. Из-за этого шебанг `#!/bin/bash` в `entrypoint.sh` перестаёт работать — ядро ищет `/bin/bash\r` и выдаёт ошибку.

**Решение:** после клонирования явно попросить Git не трогать переносы:
...
Либо настроить глобально (раз и навсегда):
```powershell
git config --global core.autocrlf input
```

После этого переклонировать репозиторий и собрать контейнер заново.
